...
export default function GreLinesApp() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      window.location.href = '/grelines/validation';
    } catch (err) {
  ...
  const isValidation = typeof window !== 'undefined' && window.location.pathname === '/grelines/validation';

  useEffect(() => {
    if (!isValidation) return;
    const t = setTimeout(() => { window.location.href = 'https://grelines-beta.vercel.app/'; }, 5000);
        ...
        <div className="max-w-2xl px-6 py-10">
          <header className="mb-10">
            <div className="flex items-baseline gap-3 flex-wrap">
              <h1 className="text-[38px] font-medium tracking-tight leading-none">GreLines</h1>
              <span className="text-[#555] text-sm">Beta Developer Version</span>
            </div>
            <p className="mt-1.5 text-xs text-[#444]">V3.0.1</p>
          </header>
  ...
  return (
    <main className="min-h-screen bg-[#0a0a0a] text-white">
      <div className="bg-[#111] border-b border-[#222] relative z-10">
        <a href="https://grelines-beta.vercel.app/" className="px-6 py-2.5 flex items-center gap-2 text-sm text-[#aaa] hover:text-white transition-colors relative z-20 cursor-pointer">
          <ArrowLeftIcon className="h-4 w-4" />
          <span>Retour vers</span>
          <img src="/assets/GreLinesLOGO.png" alt="GreLines" className="h-4 ml-1" />
        </a>
      </div>
        ...
        <form ref={formRef} onSubmit={handleSubmit} className="space-y-7" autoComplete="off">
          ...
          <section>
            ...
            <div className="rounded-[20px] bg-[#141414] border border-[#222] px-5 py-4">
              <p className="text-[15px] font-medium text-white">GreLines Website</p>
